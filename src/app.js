...
(function () {
  ...
  function persoRefFor(date) {
    ...
    var idx;
    if (p.mode === "aleatoire") {
      // tirage déterministe par jour, différent de la veille
      var seed = dayNumber(date);
      idx = hash(seed + "|" + p.livre + "|" + (p.chapitre || 0)) % pool.length;
      var prev = hash((seed - 1) + "|" + p.livre + "|" + (p.chapitre || 0)) % pool.length;
      if (pool.length > 1 && idx === prev) idx = (idx + 1) % pool.length;
    } else {
      idx = ((p.pos || 0) + (dayNumber(date) - (p.jour0 || dayNumber(new Date())))) % pool.length;
      if (idx < 0) idx += pool.length;
    }
    return { ref: pool[idx], total: pool.length, index: idx };
  }
  ...
  function viewJour() {
    ...
    // découvertes : versets déjà rencontrés dans le passage choisi
    if (usePerso && offset === 0) {
      if (!S.perso.vus) S.perso.vus = [];
      if (S.perso.vus.indexOf(ref) === -1) { S.perso.vus.push(ref); save(); }
    }
  ...
  document.addEventListener("click", function (e) {
    var t = e.target.closest("[data-tab],[data-fav],[data-note],[data-share],[data-speak]," +
      ...
      "[data-off],[data-size],[data-mode2],[data-ndel]");
    if (!t) return;
